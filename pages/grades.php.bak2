...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'load_summary') {
        ...
        $sub = (int)$_POST['subject_id'];
        $sql = "SELECT
            CONCAT(s.st_lastname,', ',s.st_name) AS FullName, s.st_gender,
            ROUND(SUM(IFNULL(p.par_one,0)+IFNULL(p.par_two,0)+IFNULL(p.par_three,0)+IFNULL(p.par_four,0)+IFNULL(p.par_five,0))
                /NULLIF(SUM(ss_p.one_max+ss_p.two_max+ss_p.three_max+ss_p.four_max+ss_p.five_max),0)*100,2) AS Participation,
            ROUND(SUM(IFNULL(w.written_one,0)+IFNULL(w.written_two,0)+IFNULL(w.written_three,0)+IFNULL(w.written_four,0)+IFNULL(w.written_five,0))
                /NULLIF(SUM(ss_w.one_max+ss_w.two_max+ss_w.three_max+ss_w.four_max+ss_w.five_max),0)*100,2) AS Written,
            ROUND(SUM(IFNULL(pf.perf_one,0)+IFNULL(pf.perf_two,0)+IFNULL(pf.perf_three,0)+IFNULL(pf.perf_four,0)+IFNULL(pf.perf_five,0))
                /NULLIF(SUM(ss_pf.one_max+ss_pf.two_max+ss_pf.three_max+ss_pf.four_max+ss_pf.five_max),0)*100,2) AS Performance,
            ROUND(SUM(IFNULL(e.score,0))/NULLIF(SUM(es.score_max),0)*100,2) AS Exam
        FROM student s
        INNER JOIN student_section ss_sec ON ss_sec.st_id=s.st_id
        LEFT JOIN class_record cr ON cr.st_id=s.st_id AND cr.sectionID=ss_sec.sectionID AND cr.sub_id=:sub
        LEFT JOIN class_record_participation crp ON crp.rec_id=cr.rec_id
        LEFT JOIN participation p ON p.par_id=crp.par_id
        LEFT JOIN score_settings ss_p ON ss_p.term=crp.term AND ss_p.component='participation'
        LEFT JOIN class_record_written crw ON crw.rec_id=cr.rec_id
        LEFT JOIN written w ON w.written_id=crw.written_id
        LEFT JOIN score_settings ss_w ON ss_w.term=crw.term AND ss_w.component='written'
        LEFT JOIN class_record_performance crpf ON crpf.rec_id=cr.rec_id
        LEFT JOIN performance pf ON pf.perf_id=crpf.perf_id
        LEFT JOIN score_settings ss_pf ON ss_pf.term=crpf.term AND ss_pf.component='performance'
        LEFT JOIN class_record_exam cre ON cre.rec_id=cr.rec_id
        LEFT JOIN exam e ON e.exam_id=cre.exam_id
        LEFT JOIN exam_settings es ON es.term=cre.term
        WHERE ss_sec.sectionID=:sec
        GROUP BY s.st_id ORDER BY s.st_gender DESC, s.st_lastname";
        $stmt = $pdo->prepare($sql);
        $stmt->execute([':sec'=>$sec,':sub'=>$sub]);
        echo json_encode($stmt->fetchAll());
        exit;
    }
}
...
function loadSummary() {
  ...
  fetch('', {method:'POST', headers:{'Content-Type':'application/x-www-form-urlencoded'},
    body: new URLSearchParams({action:'load_summary',section_id:sec,subject_id:sub})})
  .then(r=>r.json()).then(data => {
    if (!data.length) {
      document.getElementById('gradeGrid').innerHTML = '<div class="alert alert-info mb-0">No grade data found.</div>';
      return;
    }
    const pct = v => (v!==null && v!==undefined && v!=='') ? `<strong>${v}%</strong>` : '<span class="text-muted">N/A</span>';
    let html = `<div class="table-responsive">
      <table class="table table-sm table-bordered table-hover mb-0">
    ...
    data.forEach((row,i) => {
      ...
      html += `<tr>
        ...
        <td>${row.FullName}</td>
        <td><span class="badge ${g}">${row.st_gender}</span></td>
        <td class="text-center">${pct(row.Participation)}</td>
        <td class="text-center">${pct(row.Written)}</td>
        <td class="text-center">${pct(row.Performance)}</td>
        <td class="text-center">${pct(row.Exam)}</td>
      </tr>`;
    });
    html += '</tbody></table></div>';
    document.getElementById('gradeGrid').innerHTML = html;
