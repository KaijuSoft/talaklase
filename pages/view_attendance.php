...
$sectionsSql = "SELECT section.sectionID, section.section, course.course_acronym FROM section INNER JOIN course ON course.course_id=section.course_id";
if ($isInstructorScoped) {
  if (empty($ownedSectionIds)) {
...
<div class="card">
  <div class="card-header d-flex align-items-center justify-content-between">
    ...
    <button class="btn btn-sm btn-outline-secondary" onclick="
      window.open('print.php?type=view_attendance&sec=<?= urlencode($filter_sec) ?>&term=<?= urlencode($filter_term) ?>&date_from=<?= urlencode($filter_from) ?>&date_to=<?= urlencode($filter_to) ?>&name=<?= urlencode($filter_name) ?>','_blank')">
  ...
  <div class="card-body">
    ...
    <form method="GET" class="mb-3">
      ...
      <div class="row g-2">
        ...
        <div class="col-md-3">
          ...
          <select class="form-select form-select-sm" name="sec">
            ...
            <?php foreach ($sections as $s): ?>
              <option value="<?= $s['sectionID'] ?>" <?= $filter_sec==$s['sectionID']?'selected':'' ?>>
                <?= htmlspecialchars($s['section']) ?> (<?= $s['course_acronym'] ?>)
              </option>
            <?php endforeach; ?>
          </select>
        </div>
        ...
        <div class="col-12 d-flex gap-2">
          ...
          <a href="?page=view_attendance" class="btn btn-outline-secondary btn-sm">
    ...
    <?php if ($filter_sec || $filter_term || $filter_from || $filter_to || $filter_name): ?>
    <div class="d-flex flex-wrap gap-2 mb-3">
      ...
      <?php endif; ?>
      <?php if ($filter_sec): ?>
        <?php foreach($sections as $s) { if($s['sectionID']==$filter_sec) { ?>
        <span class="badge bg-secondary-subtle text-secondary border">
          <i class="bi bi-grid me-1"></i><?= htmlspecialchars($s['section']) ?>
        </span>
        <?php } } ?>
      <?php endif; ?>
      <?php if ($filter_term): ?>
